...
namespace gfx {
  ...
  bool almost_equal(scalar_type lhs, scalar_type rhs, double delta = DEFAULT_DELTA) {
    assert(delta > 0.0);
    if (lhs == rhs) {
      ...
      double lhs_double = static_cast<double>(lhs), rhs_double = static_cast<double>(rhs), signed_difference = lhs_double - rhs_double, positive_difference = std::fabs(signed_difference);
